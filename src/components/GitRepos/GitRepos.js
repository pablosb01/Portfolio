...
export default function GitRepos() {
  ...
  const username = "pablosb01"; // Cambia aquí el nombre de usuario si es necesario
  ...
  const fetchGitHubUserData = async (username) => {
    ...
    try {
      ...
      setRepos(reposData); // Actualiza el estado con los repositorios
    } catch (error) {
      console.error(error);
    } finally {
  ...
  }, [username]); // Solo se ejecuta al montar el componente o cambiar el nombre de usuario
